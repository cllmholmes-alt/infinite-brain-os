...
Load `pillars/local-first-no-cloud-dependency.md`.

**Tool architecture** (how the upscaler, interpolator, and ComfyUI interlink):
Load the relevant file in `concepts/` as it is created, and `repo-registry/` entries for
per-component posture.

**Pipeline design decisions** (why a direction was chosen):
